...
| 后端开发框架  | Spring Boot 3.2.4   | [SpringBoot官网](https://spring.io/projects/spring-boot/#learn)                                            | 基于 Spring Boot 3.2.4 + Spring Cloud 2023.0.1 + Spring Cloud Alibaba 2023.0.1.0 的微服务架构（Java 17，javax→jakarta 迁移完成）                                                                                |
...
| 分布式事务框架 | Seata               | [Seata文档](https://seata.apache.org/zh-cn/)                                                               | 微服务架构下提供高性能和简单易用的分布式事务服务（Seata server 已通过 docker 编排落地，客户端 2.0.0）。                                                                                                                                        |
...
| 分布式链路追踪 | Micrometer Tracing + Zipkin + Brave | [Micrometer Tracing文档](https://micrometer.io/docs/tracing)                                      | 基于 Micrometer Tracing(1.2.x) + Brave 桥接 + Zipkin Reporter 的跨服务链路追踪，由 TracingConfig 向 OpenFeign 调用注入 trace 上下文；指标经 micrometer-registry-prometheus 暴露，Prometheus/Grafana/Zipkin 均由 docker 编排                                                                       |
...
| 部署方式    | Docker              | [Docker文档](https://docs.docker.com/)                                                                     | 基于 Docker Compose 一键编排（Nacos/MySQL/Redis/网关/各微服务），支持按 profile 裁剪启动范围以适配小内存服务器；密钥经 .env 外部化注入，无明文入库                                                                                                   |
| 国际化     | Spring Message（MessageSource） | [SpringMVC文档](https://docs.spring.io/spring-framework/docs/current/reference/html/web.html#mvc)          | Spring 标准国际化方案（MessageSource）                                              
...
- **Nacos 版本**：docker 编排中 `nacos/nacos-server` 固定为 `2.3.2`（`docker/nacos/dockerfile`、`docker-compose*.yml`），与微服务侧 `nacos-client` 由 `pom.xml` 的 `alibaba.nacos.version=2.3.2`（`spring-cloud-alibaba 2023.0.1.0` 管理）引入的版本**一致**，gRPC 端口 9848/9849 不变。
